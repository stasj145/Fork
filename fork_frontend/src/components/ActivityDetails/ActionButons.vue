...
<template>
  <div class="button-root" v-if="selectedActivity">
    ...
    <button
      ...
      @click="deleteFood(selectedActivity)"
    ...
    >
      ...
      <ErrorModal
        ...
        message="Unable to delete activity item. Most likely you are trying to delete an item that is still being referenced in a log."
        :details="errorDetails"
      ></ErrorModal>
    </button>
    <button
      class="btn-save"
      @click="saveFood(selectedActivity)"
      v-if="isEditing"
      :disabled="saving || deleting"
    >
    ...
    <button class="btn-eat" @click="emitAddMode()" v-if="!isEditing" :disabled="saving || deleting">
...
async function deleteFood(ActivityToDelete: Activity) {
  ...
  if (!showDeletionError.value) {
    ...
    emit('activity-deleted', ActivityToDelete.id)
  }
  deleteConfirmed.value = false
}
</script>
